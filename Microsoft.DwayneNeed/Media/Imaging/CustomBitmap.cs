...
namespace Microsoft.DwayneNeed.Media.Imaging
{
    public class CustomBitmap : BitmapSource, IDisposable
    {
        ...
        {
            ValidateArrayAndGetInfo(pixels, out int elementSize, out int bufferSize, out Type elementType);

            if (offset < 0) throw new ArgumentOutOfRangeException("offset");

            // We accept arrays of arbitrary value types - but not reference types.
            if (elementType == null || !elementType.IsValueType) throw new ArgumentException("pixels");
        ...
        public sealed override void CopyPixels(Int32Rect sourceRect, IntPtr buffer, int bufferSize, int stride)
        {
            ...
            if (sourceRect.X < 0) throw new ArgumentOutOfRangeException("sourceRect");

            if (sourceRect.Width < 0) throw new ArgumentOutOfRangeException("sourceRect");

            if (sourceRect.X + sourceRect.Width > PixelWidth) throw new ArgumentOutOfRangeException("sourceRect");

            if (sourceRect.Y < 0) throw new ArgumentOutOfRangeException("sourceRect");

            if (sourceRect.Height < 0) throw new ArgumentOutOfRangeException("sourceRect");

            if (sourceRect.Y + sourceRect.Height > PixelHeight) throw new ArgumentOutOfRangeException("sourceRect");
            ...
            checked
            {
                ...
                if (stride < minStrideInBytes) throw new ArgumentOutOfRangeException("stride");
        ...
        protected void RaiseDownloadFailed(ExceptionEventArgs e)
        {
            ...
            if (downloadFailed != null) downloadFailed(this, e);
        }
        ...
        protected void RaiseDecodeFailed(ExceptionEventArgs e)
        {
            ...
            if (decodeFailed != null) decodeFailed(this, e);
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                FieldInfo _wicSource = GetType().GetField("_wicSource", BindingFlags.NonPublic | BindingFlags.Instance);
