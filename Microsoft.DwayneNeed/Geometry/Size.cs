    ...
    public struct Size<T> : ISize<T>
    {
        ...
        public Size(IEnumerable<T> extents)
        {
            ...
            foreach (T extent in extents)
            {
                ...
                if (nExtent < nZero) throw new InvalidOperationException("Size extents may not be negative.");
            }

            _extents = extents.ToArray();
        }

        public int Dimensions => _extents == null ? 0 : _extents.Length;

        public T GetExtent(int dimension)
        {
            if (dimension < 0 || _extents == null || dimension >= _extents.Length) throw new IndexOutOfRangeException();
