...
namespace Microsoft.DwayneNeed.MDI
{
    public class MdiFloater : Window
    {
        ...
        public void AdjustWindowRect(MdiWindow window, Vector delta, MdiWindowEdge interactiveEdges)
        {
            ...
            if (Content != window) throw new ArgumentException("Window does not belong to this floater.", "window");
            ...
            {
                if ((interactiveEdges & MdiWindowEdge.Left) != 0) screenRect.X = screenRect.Right - window.MinWidth;
                screenRect.Width = window.MinWidth;
            }

            if (window.MinHeight > screenRect.Height)
            {
                if ((interactiveEdges & MdiWindowEdge.Top) != 0) screenRect.Y = screenRect.Bottom - window.MinHeight;
