    ...
    internal class MdiWindowCollection : ObservableCollection<MdiWindow>
    {
        ...
        public void BringToFront(MdiWindow window, WindowState windowState)
        {
            ...
            if (windowState == WindowState.Minimized)
            {
                ...
                {
                    if (Items[newIndex] == window) foundSelf = true;
                    if (MdiPanel.GetWindowState(Items[newIndex]) != WindowState.Minimized) break;
                }

                if (foundSelf) newIndex--;
            }
        ...
        {
            foreach (MdiWindow window in Items) window.CoerceValue(dp);
        }
    }
}