...
namespace Microsoft.DwayneNeed.Threading
{
    public static class UIThreadPool
    {
        ...
        private static readonly Dictionary<Dispatcher, Tuple<bool, int>> _poolThreads =
        ...
        public static int MinThreads
        {
            ...
            set
            {
                lock (_guard)
                {
                    ...
                    while (_numThreads < _minThreads) AddNewThreadToPool();
                }
            }
        }
        ...
        public static UIThreadPoolThread AcquireThread()
        {
            lock (_guard)
            {
                ...
                var dispatcher = GetLeastReferencedThread(true);
                ...
                if (dispatcher == null) throw new InvalidOperationException("Unable to acquire worker thread.");
        ...
        internal static void DecrementThread(Dispatcher dispatcher)
        {
            lock (_guard)
            {
                ...
                if (refCount == 0 && inPool && _numThreads > _minThreads)
                    RemoveThreadFromPool(dispatcher);
                else if (refCount == 0 && !inPool) DestroyWorkerThread(dispatcher);
            }
        }
        ...
        private static Dispatcher AddNewThreadToPool()
        {
            ...
            {
                Name = "UIThreadPoolThread #" + threadId
            };
        ...
        {
            Debug.Assert(_poolThreads.ContainsKey(dispatcher));
            Debug.Assert(_poolThreads[dispatcher].Item1);
